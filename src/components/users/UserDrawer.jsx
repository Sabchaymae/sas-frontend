...
const UserDrawer = ({ isOpen, onClose, onSubmit, initialData, availableRoles = [], onRefreshRoles }) => {
  ...
  const selectOptions = useMemo(() => {
    ...
    return options;
  }, [availableRoles, initialData?.role, formData.role]);
  ...
  const handleAddNewRole = async () => {
    ...
    if (exists) {
      ...
      try {
        ...
        // Create the role dynamically in the backend's access management database
        ...
        if (response && response.success) {
          // Select the new role slug
          const newRoleSlug = response.role.slug || trimmed.toLowerCase();
          setFormData(prev => ({ ...prev, role: newRoleSlug }));
          
          // Refresh the parent's roles list so it updates everywhere immediately
          if (onRefreshRoles) {
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 md:px-8 pb-8 space-y-6 pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Date de naissance" type="date" name="dateNaissance" value={formData.dateNaissance} onChange={handleChange} max={maxDate} />
