        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="bg-[#1428C9] p-6 text-white relative">
              ...
              <h2 className="text-xl font-bold mb-4">Détails du pointage</h2>
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center font-bold text-xl border border-white/30">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-white/70 text-sm">{data.date}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50">
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Total travaillé</p>
                  <p className="text-2xl font-black text-[#111827]">{data.total}</p>
                </div>
                <div className="bg-orange-50/50 p-4 rounded-2xl border border-orange-100/50">
                  <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest mb-1">Statut final</p>
                  <p className="text-2xl font-black text-[#111827]">{data.status}</p>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Journaux d'activité</h4>
                
                <div className="relative space-y-6 before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gray-100">
                  {data.logs && data.logs.length > 0 ? data.logs.map((log, idx) => (
                    <div key={idx} className="relative pl-12">
                      <div className={`absolute left-0 top-1 w-[36px] h-[36px] rounded-full border-4 border-white flex items-center justify-center shadow-sm ${
                        log.type.includes('Entrée') ? 'bg-green-500' : 
                        log.type.includes('Sortie') ? 'bg-red-500' : 'bg-blue-500'
                      }`}>
                        <Clock size={14} className="text-white" />
                      </div>
                      <div>
                        <div className="flex items-center justify-between">
                          <p className="font-bold text-[#111827]">{log.type}</p>
                          <p className="text-sm font-black text-[#1428C9] bg-blue-50 px-2 py-0.5 rounded-md">{log.time}</p>
                        </div>
                        <p className="text-xs text-gray-500 mt-0.5">{log.device || 'Borne Principale'}</p>
                      </div>
                    </div>
                  )) : (
              ...
              <div className="mt-8 pt-6 border-t border-gray-100">
                <button 
                  onClick={onClose}
                  className="w-full py-4 bg-gray-50 text-gray-600 font-bold rounded-xl hover:bg-gray-100 transition-all active:scale-95"
                >
                  Fermer
                </button>
              </div>
...
<<<<<<< HEAD
  ...
  const filteredData = useMemo(() => {
    return attendances.filter(item => {
      ...
      // Period filter
      const itemDate = item.date; // Format DD/MM/YYYY
