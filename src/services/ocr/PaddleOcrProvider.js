...
class PaddleOcrProvider {
  ...
  async extractCin(file, onProgress) {
    try {
      ...
      // Nginx location `/api/ocr/` strips that prefix before forwarding to FastAPI.
      // FastAPI listens on `/api/v1/scan-cin`, so the full gateway path is:
      //   /api/ocr/  +  api/v1/scan-cin  →  FastAPI receives  /api/v1/scan-cin  ✓
